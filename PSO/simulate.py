...
def create_sim_file(particle, i, j):
    ...
    #var_L = str(L)
    os.chdir( os.path.normpath(read_data()['paths']['results']+""))
    
    f = open("simulacion.py", "w")   #abre un archivo para escribir

    direccion_dibujo = '"' + read_data()['paths']['asys_save_def'] + read_data()['values']['project_name'] \
                + '.aedt"'
...
def read_simulation_results(i,j):
    ...
    direccion_graficas_s11= read_data()['paths']['results']+read_data()['info']['ID']+"/figures/S11"+"_" + str(i)+"_"+str(j)
